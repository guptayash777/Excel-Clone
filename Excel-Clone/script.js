...
//initialize default prop of cells
...
function selectCell(ele, e, topCell, bottomCell, leftCell, rightCell) {
    ...
    //add prop of last selected cell in the header
    changeHeader(getRowCol(ele));   //args are passed as the return val of getrowcol func
}
//make the code two way
function changeHeader([rowId, colId]) {
    ...
    //remove default alignment of selected cell
    $(".alignment.selected").removeClass("selected");
    //add the alignment of last selected cell from header
    ...
    //change font family of font style tag in menu bar
...
let startCell = {}; //empty object declared
let endCell = {};
let scrollXRStarted = false;
let scrollXLStarted = false;
$(".input-cell").mousemove(function (e) {
    //e.buttons == 1 implies left-click is done in mousemove func
    e.preventDefault();
    if (e.buttons == 1) {
...
/*using mouseenter to get end cell as its better than mousemove 
as mousemove stores the movement in same cell as well
but we only need movement to different cells*/
...
function selectAllBetweenCells(start, end) {
    //from min row to max row
    $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected");
    for (let i = Math.min(start.rowId, end.rowId); i <= Math.max(start.rowId, end.rowId); i++) {
        //from min col to max col
        for (let j = Math.min(start.colId, end.colId); j <= Math.max(start.colId, end.colId); j++) {
            // selectCell(ele,e,topCell,bottomCell,leftCell,rightCell)
            // `#row-${i}-col-${j}`)[0] implies 'this"
            // {"ctrlKey": true} for func to execute
...
$(".alignment").click(function (e) {
    ...
    $(".alignment.selected").removeClass("selected");
    $(this).addClass("selected");   //add selected to curr cell
    $(".input-cell.selected").css("text-align", alignment);
    // $(".input-cell.selected").each(function(data){
...
$(".menu-selector").change(function (e) {
    ...
    //if value is a number than convert to integer
    // NaN: not a number
    if (!isNaN(value)) {
...
function updateCellData(property, value) {
    ...
    if (value != defaultProperties[property]) {
        ...
        $(".input-cell.selected").each(function (index, data) {
            let [rowId, colId] = getRowCol(data);
            if (cellData[selectedSheet][rowId - 1] && cellData[selectedSheet][rowId - 1][colId - 1] != undefined) {
                cellData[selectedSheet][rowId - 1][colId - 1][property] = value;
...
function addSheetEvents() {
    $(".sheet-tab.selected").on("contextmenu", function (e) {
        //when contextmenu(in which inspect is present) is 
        // called on sheet tab then this function will work 
        // instead of context menu
        e.preventDefault();
        ...
        //pageX gives x coordinate of point where the cursor is clicked
...
function renameSheet() {
    ...
    //check if name is not empty and pre-existing
    if (newSheetName && !Object.keys(cellData).includes(newSheetName)) {
        ...
        $(".sheet-modal-parent").remove();      //remove the modal
    } else {
        //pre-existing name or empty input
        $(".rename-error").remove();
        $(".sheet-modal-input-container").append(`
            <div class = "rename-error"> Sheet Name is not 
            valid or Sheet already exists! </div>
        `)
    }
}
...
function deleteSheet() {
    ...
    if (sheetIndex == 0) {
        ...
        //.next() gives jquery obj
        //we use [0] to convert jQuery $(this) to normal this
    } else {
        //select previous sheet
        selectSheet(currSelectedSheet.prev()[0]);
        //.prev() gives jquery obj
        //we use [0] to convert jQuery $(this) to normal this
    }
    ...
    $(".container").append(`<div class="sheet-modal-parent">
                                <div class="sheet-rename-modal">
                                    ...
                                    <div class="sheet-modal-input-container">
                                        ...
                                        <input type="text" class="sheet-modal-input" value="${$(".title")}"/>
                                    </div>
                                    <div class="sheet-modal-confirmation">
